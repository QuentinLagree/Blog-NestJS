...
import { CreatePackageMailerService, MailerPackageServiceMockType } from "../mocks/services/auth/create_mailer_package.service.mocks";
import * as HtmlReader from "src/usecases/utils/mail/HTML_reader";
...
describe("MailService", () => {
    ...
    describe("MailService - sendEmailToken()", () => {
        ...
        const verificationEmailData = { email: 'test@mail.com', code: 'token', expired_at: new Date() };
        ...
        it("should send an email when everything is OK", async () => {
            ...
            await expect(service.sendEmailToken(verificationEmailData, url as any)).resolves.toBeUndefined();
        ...
        it("should throw FailSendingMail if HTML file reading fails", async () => {
            ...
            await expect(service.sendEmailToken(verificationEmailData, url as any)).rejects.toThrow();
            expect(readHTMLFile).toHaveBeenCalled();
