import { CreatePrismaServiceMock, PrismaUserServiceMockType } from "src/usecases/mocks/services/database/create.prisma.service.mocks";
...
describe("TokenService", () => {
  ...
  describe("TokenService - generate()", () => {
    ...
    it("should return a new random token", async () => {
      ...
      const randomToken: string = Buffer.from('a'.repeat(16), 'utf-8').toString('hex');

      jest.spyOn(require('crypto'), 'randomBytes').mockReturnValue(Buffer.from('a'.repeat(16), 'utf-8'));
      ...
      expect(_prisma.verificationTokens.count).toHaveBeenCalledWith({ where: { code: randomToken } });
    });
    ...
    it("should return a new random token when token is already in database", async () => {
      ...
      expect(_prisma.verificationTokens.count).toHaveBeenNthCalledWith(1, { where: { code: randomToken } });
      expect(_prisma.verificationTokens.count).toHaveBeenNthCalledWith(2, { where: { code: randomToken } });
    });
    ...
    it("should call count with correct params for different tokens", async () => {
      ...
      expect(_prisma.verificationTokens.count).toHaveBeenNthCalledWith(1, { where: { code: randomToken1 } });
      expect(_prisma.verificationTokens.count).toHaveBeenNthCalledWith(2, { where: { code: randomToken2 } });
    });
  ...
  describe("TokenService - set()", () => {
    ...
    it("Should return a new verificationEmail with new Token", async () => {
      ...
      let verificationEmailMock: VerificationTokens = createVerificationEmailMock();
      _prisma.verificationTokens.findFirst.mockResolvedValue(null);
      _prisma.verificationTokens.create.mockResolvedValue(verificationEmailMock)
      ...
      await expect(_prisma.verificationTokens.findFirst()).resolves.toEqual(null)
      expect(_prisma.verificationTokens.findFirst).toHaveBeenCalledWith({ where: { email: dto.email } })
    })
    ...
    it("Should delete token if expired and create new one", async () => {
      ...
      expect(_prisma.verificationTokens.findFirst).toHaveBeenCalledTimes(1);
      expect(_prisma.verificationTokens.delete).toHaveBeenCalledWith({ where: { email: dto.email } });
      expect(_prisma.verificationTokens.create).toHaveBeenCalledWith({ data: dto });
    });
  ...
  describe("TokenService - delete()", () => {
    ...
    it("Should return error when delete method throw error", async () => {
      const validEmail = createVerificationEmailMock();
      _prisma.verificationTokens.delete.mockRejectedValue(new Error(FATAL_ERROR));

      const result = service.delete(validEmail.email)
      
      await expect(result).rejects.toThrow(Error)
      expect(_prisma.verificationTokens.delete).toHaveBeenCalledTimes(1)
      await expect(_prisma.verificationTokens.delete).rejects.toThrow(new Error(FATAL_ERROR))
      expect(_prisma.verificationTokens.delete).toHaveBeenCalledWith({where: {email: validEmail.email}})
    })
  })
  ...
  describe("TokenService - assertVerificationTokenIsValid()", () => {
    ...
    it("Shoud return void when token and email are valid", async () => {
      ...
      let result = service.assertVerificationTokenIsValid(verifEmail.email, TOKEN.add(verifEmail.code));
    ...
    it("Shoud return a TokenExpiredOrInvalidException when email is invalid", async () => {
      ...
      let result = service.assertVerificationTokenIsValid(otherVerifEmail.email, TOKEN.add(otherVerifEmail.code));
