...
describe('AuthService', () => {
  ...
  describe("AuthService - login()", () => {
    it("shoud return a user", async () => {
      ...
      jest.spyOn<AuthService, any>(service, "comparePassword").mockResolvedValue(true)
    ...
    describe("AuthService - setUserSession()", () => {
      ...
      it("should throw UserAlreadyActiveSession if session already exists", () => {
        ...
        const user = createUserMock();
        expect(() => service.setUserSession(sessionMock as any, user)).toThrow(UserAlreadyActiveSession);
      });
    })
  })
