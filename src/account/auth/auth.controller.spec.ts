...
import { makeMessage } from "src/usecases/utils/logger.utils";
...
describe('AuthController Tests', () => {
  ...
  describe("AuthController - status()", () => {
    ...
    it('Should return a success message when session is active', async () => {
      ...
      await expect(result).resolves.toEqual({message: "La session est bien active.", data: { loggedIn: true, user: userMock }})
    })
   ...
   describe("AuthController - login()" , () => {
    ...
    beforeEach(() => {
      ValidDto = jest.spyOn(userDtoModule, "dtoIsValid");
      sessionMock = {
      get: jest.fn(),
      set: jest.fn(),
    }
    ...
    it("Should return a successful login message", async () => {
      ...
      await expect(result).resolves.toEqual({message: "La connection est un succès.", data: user})
    });
    ...
    it("Should return an error message when invalid dto", async () => {
      ...
      await expectHttpExceptionWithMessage(
        ...
        makeMessage("User logged failed !", "Tous les champs sont requis.", error))
    });
    ...
    it("Should return an error message when user have already session", async () => {
      ...
      await expectHttpExceptionWithMessage(
        ...
        HttpStatus.CONFLICT,
        makeMessage("User logged failed (already logged)", "Vous êtes déjà connecté...", null),
        _auth.login,
        1
      )
    })
   ...
   describe("AuthController - register()", () => {
    ...
    it("Should return a successful register message", async () => {
      ...
      let new_user: Partial<User> = createUserMock({ ...dto, 
        id: 2, 
      ...
      expect(result).resolves.toEqual({
        message: "L'enregistrement de ton compte s'est déroulé avec succès. Maintenant tu peux te connecter.",
        data: new_user
      })
    ...
    it("Should return an error message when user already exist with email", async () => {
      ...
      await expectHttpExceptionWithMessage(
        ...
        HttpStatus.CONFLICT,
        makeMessage("User created failed", "Cette email est déjà utiliser, si vous avez déjà un compte, vous pouvez vous connecter.", null),
        _user.create,
        1
      )
    })
    ...
    it("Should return an error message when user already exist with pseudo", async () => {
      ...
      await expectHttpExceptionWithMessage(
        () => controller.register(dto),
        HttpStatus.CONFLICT,
        makeMessage("User created failed", "Ce nom d'utilisateur est déjà utilisé.", null),
        _user.create,
        1
      )
    })
