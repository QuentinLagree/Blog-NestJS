import { BadRequestException, Body, Controller, Get, HttpException, HttpStatus, NotFoundException, Post, Query, Req, UseInterceptors, UsePipes, ValidationError, ValidationPipe } from "@nestjs/common";
import { ApiBody, ApiTags } from "@nestjs/swagger";
import { User, VerificationTokens } from "@prisma/client";
import { FastifyRequest } from "fastify";
import { RESET_PASSWORD_ROUTE } from "src/constants/routes";
import { dtoIsValid, UserEmail, UserLoginCredentials, UserPasswordFields } from "src/database/entities/user/user.dto";
...
export class PasswordRecoveryController {
    ...
    async confirmResetToken(
        ...
        if (!TOKEN.hasValid(token)) {
            throw new HttpException(makeMessage("Token Invalid Format", "Le token de resiliation de votre mot de passe n'est pas dans le bon format.", null), HttpStatus.BAD_REQUEST)
        }
        ...
        try {
            ...
            return makeMessage("", "Succés lors du chargement de la page, veuillez entrer votre ancien mot de passe et ensuite entrer le nouveau mot de passe.",
                { email, token: token_id }, { log: false })

        } catch (error) {

            switch (true) {
                case error instanceof TokenExpiredOrInvalidException:
                    throw new HttpException(makeMessage("Get with token invalid", "Le token est expiré ou invalide.", null), HttpStatus.UNAUTHORIZED)
    ...
    @Post("reset")
    async changePassword(@Body() passwordManagment: UserPasswordFields): Promise<Message<isFieldsInvalid | null | User>> {
        try {
            await checkFieldIsEmpty(passwordManagment, UserPasswordFields)
            await this._auth.login({ email: passwordManagment.email, password: passwordManagment.old_password });

            await this._auth.throwAnNotSamePasswordExceptionIfNotSame(passwordManagment.password, passwordManagment.confirm_password)


            const updated_user = await this._user.update({ email: passwordManagment.email }, { password: passwordManagment.confirm_password });
            return makeMessage("updated user password", "La modification de votre mot de passe est un succé, vous pouvez désormais vous connectez.", updated_user);

        } catch (error) {

            switch (true) {
                case error instanceof isFieldsInvalid:
                    throw new HttpException(makeMessage("User Password Field Empty (password_reset)", "Tous les champs de mot de passe sont requis.", error), HttpStatus.BAD_REQUEST)

                case error instanceof PasswordNotSameException:
                    throw new HttpException(makeMessage("", "Les deux mot de passes doivent correspondre.", null, { log: false }), HttpStatus.BAD_REQUEST)
    ...
    @Post("forgot")
    async requestPasswordReset(@Body() emailDto: UserEmail, @Req() request: FastifyRequest): Promise<Message<null | isFieldsInvalid>> {

        try {
            const generated_token = await this._token.generate();
    
    
                        if (!TOKEN.hasValid(generated_token)) throw new TokenExpiredOrInvalidException();
            ...
            return makeMessage("Generate token and Send email successful", `Vous allez recevoir sur ${email} pour réinitialiser votre mot de passe.`, null);
            ...
            switch (true) {
                ...
                case error instanceof isFieldsInvalid:
                    throw new HttpException(makeMessage("User password request failed (reset_password) !", "Le champ de l'email est requis.", error), HttpStatus.BAD_REQUEST)

                case error instanceof TokenExpiredOrInvalidException:
                    throw new HttpException(makeMessage("Token Invalid Format", "Le token de resiliation de votre mot de passe n'est pas dans le bon format.", null), HttpStatus.BAD_REQUEST)

                case error instanceof BadRequestException:
                    throw new HttpException(makeMessage("User already exist in table VerificationEmail", "Vous avez déjà fais une demande contactez un administrateur.", null), HttpStatus.CONFLICT)
                case error instanceof FailSendingMail:
                    throw new HttpException(makeMessage("Error: send email failed", "Une erreur est survenue lors de l'envoie de l'email pour votre demande.", null), HttpStatus.INTERNAL_SERVER_ERROR)
