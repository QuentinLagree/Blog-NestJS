...
describe('PasswordRecoveryController Tests', () => {
    ...
    beforeEach(async () => {
        ...
        controller = app.get<PasswordRecoveryController>(PasswordRecoveryController);
    });
    ...
    describe("PasswordRecoveryController - confirmResetToken", () => {
        ...
        it("Should return a success message", async () => {
            ...
            await expect(result).resolves.toEqual({
                message: "Succés lors du chargement de la page, veuillez entrer votre ancien mot de passe et ensuite entrer le nouveau mot de passe.",
                data: { email, token: TOKEN.add(code) }
            })
            expect(_token.assertVerificationTokenIsValid).toHaveBeenCalledTimes(1)
            await expect(_token.assertVerificationTokenIsValid()).resolves.toBe(undefined);
            expect(_token.assertVerificationTokenIsValid).toHaveBeenCalledWith(email, TOKEN.add(code))
        })
        ...
        it("Should return a error when token is undefined", async () => {
            ...
            await expectHttpExceptionWithMessage(
                ...
                makeMessage("Token Invalid Format", "Le token de resiliation de votre mot de passe n'est pas dans le bon format.", null),
            )
        ...
        it("Should return a error when token is expired", async () => {
            ...
            _token.assertVerificationTokenIsValid.mockRejectedValue(new TokenExpiredOrInvalidException());
            
            await expectHttpExceptionWithMessage(
                () => controller.confirmResetToken(code, email),
                HttpStatus.UNAUTHORIZED,
                makeMessage("Get with token invalid", "Le token est expiré ou invalide.", null)
            )
            expect(_token.assertVerificationTokenIsValid).toHaveBeenCalledTimes(1)
            await expect(_token.assertVerificationTokenIsValid()).rejects.toThrow(TokenExpiredOrInvalidException);
            expect(_token.assertVerificationTokenIsValid).toHaveBeenCalledWith(email, TOKEN.add(code))
        })
        ...
        it("Should return a fatal error when error unexcepted crash", async () => {
            ...
            await expectHttpExceptionWithMessage(
                ...
                makeMessage("Fatal Error", "Une erreur est survenue, essayer de contacter l'administrateur ou réessayer ultérieurement.", error, { level: "Fatal" })
            )
        })
    })

    describe("PasswordRecoveryController - changePassword()", () => {
        let checkIfFieldsEmpty: jest.SpyInstance = jest.spyOn(emptyFieldsModule, "checkFieldIsEmpty");
        ...
        it("Should return a success message to update password with new record of user", async () => {
            ...
            const result: Promise<Message> = controller.changePassword(passwordsManager);
            await expect(result).resolves.toEqual({
                message: "La modification de votre mot de passe est un succé, vous pouvez désormais vous connectez.",
                data: newUser
            })
            ...
            expect(_auth.login).toHaveBeenCalledWith({
                email: passwordsManager.email, password: passwordsManager.old_password
            })
            ...
            expect(_user.update).toHaveBeenCalledWith(
                { email: passwordsManager.email }, { password: passwordsManager.confirm_password }
            )
        })
        ...
        it("Should return an error when fields are empty", async () => {
            ...
            checkIfFieldsEmpty
                .mockRejectedValue(new isFieldsInvalid(errors));

                await expectHttpExceptionWithMessage(
                    () => controller.changePassword(passwordsManager),
                    HttpStatus.BAD_REQUEST,
                    makeMessage("User Password Field Empty (password_reset)", "Tous les champs de mot de passe sont requis.", expect.any(isFieldsInvalid))
                )
            expect(checkIfFieldsEmpty).toHaveBeenCalledTimes(1)
            await expect(checkIfFieldsEmpty).rejects.toThrow(isFieldsInvalid);
            expect(checkIfFieldsEmpty).toHaveBeenCalledWith(passwordsManager, userDtoModule.UserPasswordFields);
        })
        ...
        it("Should return an error when user is not found", async () => {
            ...
            expect(_auth.login).toHaveBeenCalledWith({ email: passwordsManager.email, password: passwordsManager.old_password })
        })
        ...
        it("Should return an error when passwords not same", async () => {
            ...
            _auth.throwAnNotSamePasswordExceptionIfNotSame.mockRejectedValue(new PasswordNotSameException());

            await expectHttpExceptionWithMessage(
                () => controller.changePassword(passwordsManager),
                HttpStatus.BAD_REQUEST,
                makeMessage("", "Les deux mot de passes doivent correspondre.", null, { log: false })
            )
            expect(_auth.login).toHaveBeenCalledTimes(1)
    ...
    describe("PasswordRecoveryController - requestPasswordReset()", () => {
        ...
        it("Should send email successfuly and return a success message for that", async () => {
            ...
            const result = controller.requestPasswordReset({ email }, request as FastifyRequest)
        ...
        it("Sould return a error when fields are empty", async () => {
            ...
            await expectHttpExceptionWithMessage(
                ...
                HttpStatus.BAD_REQUEST,
                makeMessage("User password request failed (reset_password) !", "Le champ de l'email est requis.", expect.any(isFieldsInvalid))
            )
            expect(checkIfFieldsEmpty).toHaveBeenCalledTimes(1)
            await expect(checkIfFieldsEmpty).rejects.toThrow(isFieldsInvalid);
            expect(checkIfFieldsEmpty).toHaveBeenCalledWith({ email }, userDtoModule.UserEmail);
        })
        ...
        it("Should a error message when user post again so the request are already send", async () => {
            ...
            await expectHttpExceptionWithMessage(
                () => controller.requestPasswordReset({email: verifEmail.email}, request as FastifyRequest),
                HttpStatus.CONFLICT,
                makeMessage("User already exist in table VerificationEmail", "Vous avez déjà fais une demande contactez un administrateur.", null)
            )
            expect(_token.generate).toHaveBeenCalledTimes(1)
            expect(_token.set).toHaveBeenCalledTimes(1)
        ...
        it("Should a error message when send mail fail", async () => {
            ...
            _token.set.mockResolvedValue(verifEmail);
            _mailer.sendEmailToken.mockImplementationOnce(() => {throw new FailSendingMail()});
            ...
            await expectHttpExceptionWithMessage(
                ...
                HttpStatus.INTERNAL_SERVER_ERROR,
                makeMessage("Error: send email failed", "Une erreur est survenue lors de l'envoie de l'email pour votre demande.", null)
            )
        })
        ...
        it("Should a error when error unexpected crash return", async () => {
            ...
            _token.set.mockResolvedValue(verifEmail);
            const error = new Error(FATAL_ERROR)
            _mailer.sendEmailToken.mockImplementationOnce(() => {throw error});
