...
import { dtoIsValid, UserDto } from '../../database/entities/user/user.dto';
...
describe('UserService', () => {
  ...
  describe('UserService - show()', () => {
    ...
    it('sould return an error message when ID is undefined', async () => {
      ...
      expect(_prisma.user.findUnique).toHaveBeenCalledWith({where: undefined})
    });
    ...
    it('should throw NotFoundExcpetion when a user is not found', async () => {
      ...
      expect(_prisma.user.findUnique).toHaveBeenCalledWith({where: {id: 999}})
    });
    ...
    it('should return a Fatal Error (database down)', async () => {
      ...
      expect(_prisma.user.findUnique).toHaveBeenCalledTimes(1)
      await expect(_prisma.user.findUnique()).rejects.toThrow(Error(FATAL_ERROR));
    });
  });
  ...
  const newUser: Partial<User> = createUserMock({ ...dto, id: 2, password: hashedPassword });
  ...
  describe('UserService - update()', () => {
    ...
    it('should update a user successfully', async () => {
      ...
      const updatedUser: Partial<User> = createUserMock({ id: 1, nom: 'Updated Name' });

      _prisma.user.findUnique.mockResolvedValue(existingUser);
      _prisma.user.update.mockResolvedValue(updatedUser);

      const result: Promise<User> = service.update({ id: 1 }, { nom: 'Updated Name' });

      await expect(result).resolves.toEqual(updatedUser);
      expect(_prisma.user.findUnique).toHaveBeenCalledWith({ where: { id: 1 } });
    ...
    it("sould return an Error when id isn't valid (decimal number)", async () => {
      await expect(service.update(1.5 as any, { nom: "NomdeFamille"})).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw error if DTO is invalid', async () => {
      ...
      await expect(service.update(invalidDto as User, { nom: "NomdeFamille"})).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw error if database down', async () => {
      ...
      await expect(service.update({id: 1}, { nom: "NomDeFamille"})).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('UserService - destroy()', () => {
    ...
    it("sould return an Error when id isn't valid (decimal number)", async () => {
      await expect(service.destroy(1.5 as any)).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw error if database down', async () => {
      ...
      await expect(service.destroy({id: 1})).rejects.toThrow(NotFoundException);
    })
  });
});
