...
import { createInvalidDto, createNewUserMock, createUserMock } from '../../usecases/mocks/create.user.mocks';
...
describe('UserController Tests', () => {
  ...
  describe('UserController - index()', () => {
    ...
    it('Should return an empty list of users', async () => {
      ...
      await expect(result).resolves.toEqual({ message: "La liste des utilisateurs est vide", data: null });
    });
  ...
  describe('UserController - show()', () => {
    ...
    it('Should return a user with unknow id from database', async () => {
      ...
      await expectHttpExceptionWithMessage(
        ...
        makeMessage(`User Not Found with id 999`,
  ...
  describe('UserController - store()', () => {
    ...
    it('should return error message if user already exists with email', async () => {
      ...
      await expectHttpExceptionWithMessage(
        ...
        HttpStatus.CONFLICT,
        makeMessage("User created failed", "Ce compte existe déjà, connectez-vous.", null),
        service.create,
        1)
    });
    ...
    it('should throw error if DTO is invalid', async () => {
      ...
      await expectHttpExceptionWithMessage(
        ...
        HttpStatus.BAD_REQUEST,
        makeMessage("User created failed !", "Les données sont incorrectes !", errors),
        service.create
      )
  ...
  describe('UserController - update()', () => {
    ...
    it('should update user with success message', async () => {
      const updatedUser: UserUpdateDto = createUserMock({ id: 1, nom: "UpdatedName" })

      service.update.mockResolvedValue(updatedUser)

      const result: Promise<Message> = controller.update(1, { nom:"UpdatedName" })

      await expect(result).resolves.toEqual({ message: "La modification de vos informations est bien sauvegardé !", data: updatedUser })
      expect(service.update).toHaveBeenCalledTimes(1)

      expect(service.update).toHaveBeenCalledWith({ id: 1 }, { nom: "UpdatedName" })
    })
    ...
    it('should throw error if DTO is invalid', async () => {
      ...
      const errors: ValidationError[] = await dtoIsValid(invalidUpdateDto, UserUpdateDto);
      expect(errors.length).not.toBe(0);
  ...
  describe('UserController - destroy()', () => {
    ...
    it('should delete user with success message', async () => {
      ...
      await expect(result).resolves.toEqual({ message: "La suppression de votre compte utilisateur est un succée !", data: null })
