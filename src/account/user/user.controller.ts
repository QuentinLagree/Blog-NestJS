import { Body, Controller, Delete, Get, HttpCode, HttpException, HttpStatus, NotFoundException, Param, Post, Put, Res, SerializeOptions, UseInterceptors, ValidationError } from "@nestjs/common";
import { ApiBody, ApiTags } from "@nestjs/swagger";
import { User } from "@prisma/client";
import { dtoIsValid, UserDto, UserUpdateDto } from "../../database/entities/user/user.dto";
...
export class UserController {
  ...
  async index(): Promise<Message<User[] | null>> {
    try {
      ...
      return (users.length == 0) ?
        makeMessage("List of all users is empty.", "La liste des utilisateurs est vide", null)
        : makeMessage("List of all users", "Liste de tous les utilisateurs", users)
    ...
    try {
      ...
      switch (true) {
        case error instanceof NotFoundException:
          throw new HttpException(makeMessage(`User Not Found with id ${type_id.value()}`,
  ...
  async store(@Body() createData: UserDto): Promise<Message<User | null | ValidationError[]>> {
    ...
    try {
      ...
      return makeMessage("User created !", "L'utilisateur est bien enregistré !", created_user)
    } catch (error) {
      switch (true) {
        case error instanceof UserAlreadyExistWithEmail:
          throw new HttpException(makeMessage("User created failed", "Ce compte existe déjà, connectez-vous.", null), HttpStatus.CONFLICT)
  ...
  @Put('/:id')
  async update(@Param('id') id: number, @Body() updateData: UserUpdateDto): Promise<Message<User | null | ValidationError[]>> {

    const errors: ValidationError[] = await dtoIsValid(updateData, UserUpdateDto)
    ...
    if (!ID.hasValid(id))
      throw new HttpException(makeMessage(
      `Error Param ID : '${id}' is invalid.`,
      "L'id doit être un nombre entier.",
      null
    ), HttpStatus.BAD_REQUEST)
    ...
    try {
      let updated_user = await this._user.update({ id: id_type.value() }, updateData)
      return makeMessage("User updated !", "La modification de vos informations est bien sauvegardé !", updated_user)
    } catch (error) {
      switch (true) {
        case error instanceof NotFoundException:
          throw new HttpException(makeMessage("User updated failed", `Ce compte n'existe pas.`, null), HttpStatus.NOT_FOUND)
  ...
  async destroy(@Param('id') id: number): Promise<Message<null>> {
    ...
    try {
      ...
      return makeMessage("User deleted !", "La suppression de votre compte utilisateur est un succée !", null)
    } catch (error) {
      switch (true) {
        case error instanceof NotFoundException:
          throw new HttpException(makeMessage("User deleted failed", `Ce compte n'existe pas.`, null), HttpStatus.NOT_FOUND)
        default:
          throw new HttpException(makeMessage("Fatal Error", "Une erreur est survenue, essayer de contacter l'administrateur ou réessayer ultérieurement.", error, { level: "Fatal" }), HttpStatus.INTERNAL_SERVER_ERROR)
      }
    }
