import { CreatePostsServiceMock } from "src/usecases/mocks/services/post/create_posts.service.mocks";
...
describe("PostsController Tests", () => {
  ...
  describe('PostController - index()', () => {
      ...
      it('Should return an empty list of users', async () => {
        ...
        await expect(result).resolves.toEqual({ message: "La liste des publications est vide", data: null });
      });
    ...
    describe('PostController - indexPublished', () => {
      ...
      afterEach(() => jest.restoreAllMocks())
      it('Should return an array of user', async () => {
        const posts = [createPostMock({ published: true}), createPostMock({ id: 2 })];
      ...
      it('Should return an empty list of users', async () => {
        ...
        await expect(result).resolves.toEqual({ message: "La liste des publications publiées est vide", data: null });
      });
    ...
    describe('UserController - show()', () => {
        ...
        it('Should return a post with unknow id from database', async () => {
          ...
          await expectHttpExceptionWithMessage(
           ...
           makeMessage(`Posts Not Found with id 999`,
    ...
    it('should delete post with success message', async () => {
      ...
      await expect(result).resolves.toEqual({ message: "La suppression de votre publication est un succée !", data: null })
    ...
    it('should return a message when post is not found', async () => {
      ...
      await expectHttpExceptionWithMessage(
        ...
        HttpStatus.NOT_FOUND,
        makeMessage("Post deleted failed", `Cette publication n'existe pas.`, null)
      )
      expect(_posts.destroy).toHaveBeenCalledTimes(1)
