...
describe('PostService', () => {
  ...
  describe('PostService - index(true) -- Published Post', () => {
    ...
    it('should return an array of post', async () => {
      ...
      _prisma.post.findMany.mockResolvedValue(users.filter(post => post.published));
    ...
    it('should return an empty list of published posts', async () => {
      ...
      _prisma.post.findMany.mockResolvedValue(posts.filter(post => post.published));
  ...
  describe('PostService - show()', () => {
    ...
    it('sould return an error message when ID is undefined', async () => {
      ...
      expect(_prisma.post.findUnique).toHaveBeenCalledWith({where: undefined})
    });
    ...
    it('should throw NotFoundExcpetion when a user is not found', async () => {
      ...
      await expect(_posts.show({ id: 999 })).rejects.toThrow(
        NotFoundException,
      );
      expect(_prisma.post.findUnique).toHaveBeenCalledTimes(1)
      await expect(_prisma.post.findUnique()).resolves.toBe(null);
      expect(_prisma.post.findUnique).toHaveBeenCalledWith({where: {id: 999}})
    });
    ...
    it('should return a Fatal Error (database down)', async () => {
      ...
      expect(_prisma.post.findUnique).toHaveBeenCalledTimes(1)
      await expect(_prisma.post.findUnique()).rejects.toThrow(Error(FATAL_ERROR));
    });
  });
  ...
  describe('PostService - destroy()', () => {
    it('should delete a post', async () => {
      ...
      expect(_prisma.post.findUnique).toHaveBeenCalledWith({ where: { id: 1 } });
    ...
    it("sould return an Error when id isn't valid (decimal number)", async () => {
      await expect(_posts.destroy(1.5 as any)).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw error if database down', async () => {
      ...
      await expect(_posts.destroy({id: 1})).rejects.toThrow(NotFoundException);
    })
  });
});
